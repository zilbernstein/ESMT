...
void __fastcall TfrmAppFileLoad::aFTPConnectExecute(TObject *Sender)
{
    ...
    {
        throw Exception ("FTP-04: Ошибка инициализации настроек FTP.\n" + e.Message);
    }

    if ( !IdFTP->Connected() )
        throw Exception ("FTP-02: Не удалось установить соединение с сервером.");
    ...
    {
        throw Exception ("FTP-05: Ошибка смены директории.\n" + e.Message);
    }
}
//---------------------------------------------------------------------------
	...
	try
    {
		...
		for ( int i = 0; i < cds->RecordCount; i++ )
        {
            if ( FileExistsCheck(cds->FieldByName("file_name")->AsString) == false )
                throw Exception("AF-12: Файл [" + cds->FieldByName("file_name")->AsString + "] не найден.");
			...
			FHandle = FileOpen(TargetFileName, 0);
			res = DateTimeToStr(FileDateToDateTime(FileGetDate(FHandle)));

            // сравнить размеры
            FileSearchResult = FindFirst(TargetFileName, 0, sr);

            if  ( 	FileSearchResult != 0
            	|| (FileSearchResult == 0 && /*sr.Size*/ IdFTP->Size(cds->FieldByName("file_name")->AsString) != fsz)
                )
            {
                ...
                if (FindFirst(TargetFileName, 0, sr) == 0)
                    ...
                    throw Exception("Файл <" + cds->FieldByName("file_name")->AsString + "> не загружен.");
		...
		{
            IdFTP->Socket->Close();
            LogEvent("FTP-11: IdFTP->Disconnect: EIdSocketError: " + e.Message);
        }
...
void __fastcall TfrmAppFileLoad::aLoadFileListExecute(TObject *Sender)
{
    ...
    {
        throw Exception ("<AF-01>: Ошибка загрузки списка системных файлов\n" + e.Message);
    }

    if ( cds->Active == false || cds->RecordCount == 0 )
    {
        throw Exception ("<AF-02>: Список системных файлов не обнаружен\n\nОбратитесь к разработчикам");
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmAppFileLoad::aLoad_FTP_ParamsExecute(TObject *Sender)
{
    ...
    {
        throw Exception ("AF-01: Не удалось получить параметры FTP-соединения:\n" + e.Message);
    }
}
