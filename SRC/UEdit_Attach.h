//---------------------------------------------------------------------------

#ifndef UEdit_AttachH
#define UEdit_AttachH
#include "cxButtons.hpp"
#include "cxCalendar.hpp"
#include "cxContainer.hpp"
#include "cxControls.hpp"
#include "cxDBExtLookupComboBox.hpp"
#include "cxDBLookupComboBox.hpp"
#include "cxDBLookupEdit.hpp"
#include "cxDropDownEdit.hpp"
#include "cxEdit.hpp"
#include "cxGraphics.hpp"
#include "cxImageComboBox.hpp"
#include "cxLabel.hpp"
#include "cxLookAndFeelPainters.hpp"
#include "cxLookupEdit.hpp"
#include "cxMaskEdit.hpp"
#include "cxMemo.hpp"
#include "cxTextEdit.hpp"
#include "dxSkinsCore.hpp"
#include "dxSkinsdxStatusBarPainter.hpp"
#include "dxStatusBar.hpp"
#include <ActnList.hpp>
#include <Classes.hpp>
#include <Controls.hpp>
#include <ExtCtrls.hpp>
#include <Menus.hpp>
#include <StdCtrls.hpp>
#include <Dialogs.hpp>
#include "cxLookAndFeels.hpp"
#include "dxSkinCaramel.hpp"
#include "cxButtonEdit.hpp"
#include "cxClasses.hpp"
#include "cxGridBandedTableView.hpp"
#include "cxInplaceContainer.hpp"
#include "cxStyles.hpp"
#include "cxTL.hpp"
#include "cxVGrid.hpp"
#include <System.Actions.hpp>
//---------------------------------------------------------------------------
struct Strct_Attach
{
    int	sheet_id;
    int attach_id;
    int group_id;
    AnsiString	group_name;
    AnsiString	attach_descr;
    AnsiString	file_name;
};
//---------------------------------------------------------------------------
class TfrmEdit_Attach : public TForm
{
__published:
	TTimer *tmr;
	TActionList *alAction;
	TAction *aSave;
	TAction *aCancel;
	TPanel *Panel6;
	TcxButton *cxButton9;
	TcxButton *cxButton10;
	TdxStatusBar *dxStatusBar1;
	TAction *aSelectFile;
	TOpenDialog *dlgOpen;
	TAction *aLoadFileExtension;
	TcxVerticalGrid *vg;
	TcxCategoryRow *cxCategoryRow1;
	TcxEditorRow *rwFileName;
	TcxEditorRow *rwDescr;
	void __fastcall tmrTimer(TObject *Sender);
	void __fastcall aCancelExecute(TObject *Sender);
	void __fastcall FormKeyPress(TObject *Sender, wchar_t &Key);
	void __fastcall aSaveExecute(TObject *Sender);
	void __fastcall aSelectFileExecute(TObject *Sender);
	void __fastcall aLoadFileExtensionExecute(TObject *Sender);
	void __fastcall rwFileNameEditPropertiesButtonClick(TObject *Sender, int AButtonIndex);

private:
	Strct_Attach* te_;
    int vDefaultActionType;
    int vDefaultResult;
public:
	TModalResult __fastcall Show( Strct_Attach* te );
    AnsiString __fastcall MultiSave(Strct_Attach* te);
	__fastcall TfrmEdit_Attach(TComponent* Owner);
};
//---------------------------------------------------------------------------
extern PACKAGE TfrmEdit_Attach *frmEdit_Attach;
//---------------------------------------------------------------------------
#endif
